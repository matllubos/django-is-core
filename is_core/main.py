...
class HomeUIISCore(UIISCore):
    ...
    Core is abstract only if your application contais rewrited HomeUIISCore.
    """
    ...
    view_classes = (
        ('index', r'^$', str_to_class(config.HOME_VIEW)),
    )
...
class RESTModelISCore(ModelISCore, RESTISCore):
    """
    ...
    def get_rest_patterns(self):
        ...
        rest_patterns.update(DoubleRESTPattern(self.get_rest_class(), self.rest_resource_pattern_class, self).patterns)
        return rest_patterns
